...
The source repository is [jdalbright/8-bit-weather](https://github.com/jdalbright/8-bit-weather). Vercel builds the linked GitHub repository with `npm run build` and serves `dist/`. Pushes to `main` deploy to production; other branches receive preview deployments. Weather requires server-only Xweather credentials; the optional briefing requires server-only OpenAI configuration. See [Xweather setup and activation](docs/xweather.md). `vercel.json` keeps the service worker fresh so installed apps can discover updates.
...
Forecasts and current conditions come from [Vaisala Xweather](https://www.xweather.com/) through this app's server. Configure a free account without paid overages; its shared monthly allowance is finite. City search continues to use [Open-Meteo](https://open-meteo.com/) and [GeoNames](https://www.geonames.org/) under their existing noncommercial terms. NOAA supplies radar independently.

Current conditions are estimates blending observations, radar, satellites, and models. The adapter maps provider codes and qualifiers into the app's weather categories. Times use the selected location's time zone; Celsius/km/h values are converted locally for Fahrenheit/mph. See [provider integration, caching, and configuration](docs/xweather.md).

Preferences, saved places, the selected location, and up to 12 recent forecast snapshots are stored locally in this browser on web, or in native Preferences on iOS. The iOS widget stores the selected place and a forecast projection in the app’s local App Group, and can refresh that place through the same Xweather backend while the app is closed. Coordinates (rounded to three decimal places for GPS selections) are sent through the backend to Xweather to request weather; search terms are sent to Open-Meteo’s geocoding service. The providers’ [Xweather privacy policy](https://www.xweather.com/privacy) and [Open-Meteo privacy policy](https://open-meteo.com/en/terms#privacy) apply to their respective requests. Native briefings use Apple Intelligence on-device when available. When cloud briefings are enabled, the website and native fallback send a limited hourly forecast, timezone, and chosen temperature units through this app’s server to OpenAI. Native fallback runs automatically when Apple Intelligence is unavailable or fails and the app is online. Coordinates and place names are excluded from those requests. Responses use `store: false`; this does not disable OpenAI’s separate abuse-monitoring retention. The app has no accounts, analytics, or ads. Settings → Clear saved data removes stored choices, forecasts, and briefings, including in-memory copies.

Current conditions and rain refresh at ten-minute cache expiry while the app is visible; forecasts and history use one-hour caches. Manual Refresh respects those same caches. Searches are debounced, superseded requests are canceled, and provider rate-limit cooldowns are respected. A GPS cache is never reused at changed coordinates. If browser storage fails, the latest forecast for the current place stays available in memory through refresh failures and offline resume during that visit.
...
The Weather briefing card summarizes the next 24 elapsed hours with a warm, practical explanation and useful recommendations. OpenAI has no fixed sentence count or hard word limit; 80–160 words is an advisory guide, with more room when changing weather warrants it. Complete paragraphs are preserved. It loads independently above the hourly forecast. The default model is `gpt-5.6-luna`, with reasoning disabled, a 1,000-token output budget, a 12-second SDK timeout, and no automatic SDK retries. Model input comes from the displayed forecast, with temperatures converted and precipitation intervals aligned in code. Unsupported weather codes count as missing data when checking coverage. No weather search or tools are enabled in OpenAI.
...
A compact pixel timeline appears below the current conditions only when rain or showers are forecast within the next hour. Tap or slide the timeline (or use the arrow keys) for the time window and expected amount in inches or millimeters. Dry weather adds no extra card. Snow alone does not trigger it.

The outlook uses Xweather's minute-by-minute liquid precipitation estimates with a 0.4 mm/hour sensitivity, equivalent to the previous 0.1 mm per 15-minute threshold. The card shows the unelapsed portion of the next-hour forecast and hides when offline, expired, or missing any required minute. Frozen or unknown precipitation cannot be treated as liquid rain. Legacy snapshots remain readable. See [interval normalization and freshness](docs/xweather.md).
