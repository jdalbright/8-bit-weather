...
test('native pull-to-refresh finishes after fetching and detaches when leaving Today', async ({ page }) => {
  ...
  await page.route('**/api/weather?**', route => {
    requests++;
    return route.fulfill({ json: apiFixture(forecastFixture(Date.now()), route.request().url()), headers: { 'access-control-allow-origin': '*' } });
  });
  await page.goto('/');
  ...
  await expect.poll(() => requests).toBe(before + 4);
