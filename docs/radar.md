...
The NOAA legends in `public/radar` are the original `BREFQCD_CT.png` and `PCPNTYP_CT.png` from `https://www.weather.gov/images/nws/radarfaq/`. Intensity markings run from −20 to 70 dBZ. The type legend's actual left-to-right labels are **WS, S, C, H, CS, ST, CT** (the FAQ prose calls the last two TS and TC). The accessible color key follows the image. Radar colors are not recolored; nearest-neighbor raster rendering avoids smoothing categories. Imagery uses 80% opacity for geographic context.
