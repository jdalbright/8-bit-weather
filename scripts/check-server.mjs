...
try {
  ...
  const weather = await import(pathToFileURL(join(output, 'api/weather.js')).href);
  assert.equal((await weather.default.fetch(new globalThis.Request('https://weather.example/api/weather'))).status, 400);
