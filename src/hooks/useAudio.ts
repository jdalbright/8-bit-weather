...
export function useAudio(preferences: Preferences, scene: SceneState) {
  ...
  const toggle = useCallback(async () => {
    ...
    try {
      ...
      if (attempt !== activity.current || !enabledRef.current || document.hidden) return;
  ...
  useEffect(() => {
    ...
    const visibility = () => {
      ...
      if (document.hidden) { setPlaying(false); void engine.current?.pause(); }
        ...
        if (attempt === activity.current && enabledRef.current && !document.hidden) setPlaying(true);
      }).catch(() => {
        if (attempt !== activity.current) return;
    ...
    return () => { invalidate(); document.removeEventListener('visibilitychange', visibility); void engine.current?.dispose(); engine.current = null; };
  }, []);
