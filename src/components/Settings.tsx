...
export default function Settings({ preferences, onChange, audio, install, systemReduced, onClear }: Props) {
  ...
  return <main id="main-content" className="utility-view settings-view">
    ...
    <section className="settings-section" aria-labelledby="install-title"><h2 id="install-title">A home for your weather</h2><p className="settings-description">Add 8-Bit Weather to your home screen. Your last forecast stays with you offline.</p><button className="pixel-button full-width" disabled={install.installed} onClick={() => void install.install()}><Icon name={install.installed ? 'check' : 'download'} size={18}/>{install.installed ? 'App installed' : install.canPrompt ? 'Install app' : 'How to install'}</button>
    ...
    <section className="settings-section" aria-labelledby="data-title"><h2 id="data-title">Your device, your data</h2><p className="settings-description">Places, preferences, and forecasts stay in this browser. Coordinates are sent to Open-Meteo to get your forecast. No account or analytics.</p>
      {!confirmClear ? <button className="text-button danger-button" onClick={() => setConfirmClear(true)}><Icon name="trash" size={16}/>Clear saved data</button> : <div className="clear-confirmation"><p>Clear saved places, forecasts, and settings from this device?</p><div><button className="pixel-button small" onClick={() => setConfirmClear(false)}>Keep my data</button><button className="pixel-button small danger" onClick={onClear}>Clear everything</button></div></div>}
    </section>
    <footer className="settings-footer"><strong>8-BIT WEATHER</strong><p>A little pixel world. Your real weather.</p><span>Version 1.0.0 · Free & noncommercial</span><p><a href="https://open-meteo.com/" target="_blank" rel="noreferrer">Weather by Open-Meteo</a> · <a href="https://open-meteo.com/en/terms" target="_blank" rel="noreferrer">Data & privacy</a></p></footer>
  </main>;
}
