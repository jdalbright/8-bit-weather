...
export function WeatherBriefing({ snapshot, units, online, now, provider = 'openai', onProviderChange }: { provider?: BriefingProvider; onProviderChange?: (provider: BriefingProvider) => void; snapshot: WeatherSnapshot; units: Units; online: boolean; now: number }) {
  ...
  return <section className="weather-briefing" aria-labelledby="briefing-title" aria-busy={loading}>
    ...
    <div role="status" aria-live="polite">
      ...
      </> : <p className="briefing-copy briefing-placeholder">{loading ? 'Putting your next 24 hours into words…' : error ? error.message : !online && !isNativeApp() ? 'Connect for a fresh weather briefing.' : !online ? 'A fresh forecast is needed for a new briefing. Connect to update your weather.' : 'A fresh, complete forecast is needed for your briefing.'}</p>}
    </div>
    ...
    {canSwitch && onProviderChange ? <button className="text-button briefing-switch" onClick={() => onProviderChange(alternative)}>Switch to {alternative === 'apple' ? 'Apple Intelligence' : 'OpenAI'}</button> : null}
    </div></div></div>
  </section>;
}
