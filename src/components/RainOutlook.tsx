...
import { rainAmount, type RainOutlookData } from '../lib/rain';
...
export function RainOutlook({ onRadar, outlook, timezone, units, now }: Props) {
  ...
  const { periods, firstRainIndex, interval } = outlook;
  ...
  const firstStart = periods[firstRainIndex].time - interval;
  const summary = firstStart <= now / 1000 ? 'Rain forecast in the next hour' : `Rain possible around ${clock(firstStart)}`;
  const detail = `${clock(active.time - interval)}–${clock(active.time)} · ${rainAmount(active.amount, units)} of rain`;
  const scale = Math.max(interval === 60 ? 1 / 15 : 1, ...periods.map(period => period.amount));
  return <section className="rain-outlook" aria-labelledby="rain-outlook-title">
    <div className="rain-heading"><h2 id="rain-outlook-title"><WeatherIcon kind="rain" size={25}/>Rain outlook</h2><span>Next hour</span></div>
    <p className="rain-summary">{summary}</p>
    <div className="rain-scale"><span>Rain per {interval / 60} min</span><span>{rainAmount(scale, units)}</span></div>
    ...
    <div className="rain-times" aria-hidden="true"><span>{clock(periods[0].time - interval)}</span><span>{clock(periods[Math.floor(periods.length / 2)].time - interval)}</span><span>{clock(periods.at(-1)!.time)}</span></div>
